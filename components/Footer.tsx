...
export default async function Footer() {
  const translate = await getTranslations("MarketingFooter");
      ...
      <div className="mx-auto max-w-[var(--sx-max-content)] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-[1.25fr_1fr]">
          <div>
            ...
            <div className="mt-6">
              <h2 className="text-sm font-medium">{translate("socials")}</h2>
          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3">
            {footerGroups.map((group) => (
              <div key={group.key}>
                <h2 className="text-sm font-medium">{translate(`groups.${group.key}`)}</h2>
