            ...
            {content.items.map((item, index) => {
              ...
              return (
                <Reveal key={item.key} delay={index * 0.07} distance={16}>
                  <Link
                    href={item.href}
                    className="group grid gap-5 rounded-[14px] border bg-card p-5 transition-[border-color,background-color,transform] duration-200 hover:-translate-y-0.5 hover:border-foreground/20 hover:bg-muted/35 sm:grid-cols-[auto_1fr_auto] sm:items-start sm:p-6"
                  >
                    <div className="flex size-11 items-center justify-center rounded-[10px] border bg-background">
                    ...
                    <ArrowUpRight className="mt-1 hidden size-5 text-muted-foreground transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5 sm:block" />
                  </Link>
                </Reveal>
              );
            })}
