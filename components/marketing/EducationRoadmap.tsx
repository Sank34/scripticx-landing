...
import type { EducationTrackKey } from "@/types/education";
import type { SiteLocale } from "@/config/languages";

const tracks = [
  "informatics",
  "machineLearning",
] as const satisfies readonly EducationTrackKey[];
const trackIcons = { informatics: Code2, machineLearning: BrainCircuit };

export function EducationRoadmap({ locale }: { locale: SiteLocale }) {
  const { roadmap, groups } = educationContent[locale];
    ...
    >
      <div className="px-5 py-24 sm:px-8 sm:py-32 lg:px-12 xl:px-16 2xl:px-24">
        <div className="max-w-3xl">
          ...
          <h2
            id="learning-roadmap-title"
            className="mt-4 text-balance text-4xl font-semibold tracking-[-0.04em] sm:text-5xl"
          >
            {roadmap.title}
          </h2>
        ...
        <div className="mt-12 space-y-10 sm:mt-16 sm:space-y-12">
          {tracks.map((track) => {
            ...
            const trackGroups = groups.tracks[track];

            return (
              <div key={track}>
                <div className="mb-5">
                  <h3 className="flex shrink-0 items-center gap-3 text-xl font-semibold tracking-[-0.025em]">
                    <span className="flex size-10 items-center justify-center rounded-[10px] border bg-muted/30">
        ...
        <div className="mt-10 flex items-start gap-4 border-t pt-7 sm:mt-12">
          ...
          <div className="max-w-3xl">
            <h3 className="text-lg font-semibold tracking-[-0.02em]">
              {roadmap.practiceTitle}
            </h3>
            <p className="mt-2 text-base leading-7 text-muted-foreground">
              {roadmap.practiceDescription}
            </p>
          </div>
        </div>
      </div>
