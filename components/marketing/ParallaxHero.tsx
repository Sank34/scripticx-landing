...
function FloatingPhoto({
  ...
  return (
    <motion.figure
      style={{ y: photoY }}
      className={`absolute hidden overflow-hidden rounded-[14px] border bg-card p-1.5 shadow-[0_20px_60px_rgba(15,23,42,0.16)] sm:block ${photo.className}`}
    >
      <div className="relative size-full overflow-hidden rounded-[9px]">
        <Image src={photo.src} alt={photo.alt} fill sizes="(max-width: 1024px) 260px, 340px" className="object-cover" />
      </div>
    </motion.figure>
  );
}
...
export default function ParallaxHero() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto mt-12 h-[29rem] max-w-[100rem] sm:absolute sm:inset-x-0 sm:top-[39rem] sm:mt-0 sm:h-[51rem] lg:top-[42rem]">
        {photos.map((photo) => (
          <FloatingPhoto key={photo.src} photo={photo} scrollYProgress={scrollYProgress} reduceMotion={reduceMotion} />
        ))}
